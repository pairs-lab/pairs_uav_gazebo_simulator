...
## Naming Rules
To reduce the Sim2Real gap, we provide topic arguments that are passed into a dynamically generated SDF file. These arguments are inserted as custom SDF tags, which are ignored by Gazebo but read by `pairs_drone_spawner` to generate the ros_gz_bridge config file. While this allows us to customize topic names so that they match those used by real-world sensors, there are certain constraints.
...
2. `lidar_gz_topic_name` - the Gazebo topic that will be bridged to ROS by pairs_drone_spawner
