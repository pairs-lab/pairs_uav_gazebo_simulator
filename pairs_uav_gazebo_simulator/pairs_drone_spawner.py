...
from pairs_uav_gazebo_simulator.utils.spawner_types import Px4MavlinkConfig
from pairs_uav_gazebo_simulator.utils.spawner_exceptions import *
from pairs_uav_gazebo_simulator.core.jinja_template_manager import JinjaTemplateManager
from pairs_uav_gazebo_simulator.core.ros_gz_bridge_manager import RosGzBridgeManager
from pairs_uav_gazebo_simulator.core.px4_mavlink_manager import Px4MavlinkManager
from pairs_uav_gazebo_simulator.core.user_input_manager import UserInputManager
from pairs_uav_gazebo_simulator.core.sdf_to_tf_publisher import SdfTfPublisherSingleton
...
def exit_handler():
        ...
        for p in glob_running_processes:
            try:
                if p.is_alive():
                    p.terminate()
                    p.join()
                    print(f'[INFO] [PairsDroneSpawner]: Process {p.pid} terminated')
                else:
                    print(f'[INFO] [PairsDroneSpawner]: Process {p.pid} finished cleanly')
            except:
                num_zombies += 1
    ...
    if rclpy.ok():
        rclpy.shutdown()
        print('[INFO] [PairsDroneSpawner]: rclpy shut down.')
...
class PairsDroneSpawner(Node):

    def __init__(self):
        super().__init__('pairs_drone_spawner')
        ...
        gazebo_simulator_path = get_package_share_directory('pairs_uav_gazebo_simulator')
    ...
    def _handle_rosparams(self) -> list[str]:
        ...
        resource_paths = [os.path.join(get_package_share_directory('pairs_uav_gazebo_simulator'), 'models')]
    ...
    def _create_tempfile_folder(self) -> str:
        ...
        temp_folder = f'pairs_gazebo_simulator_{time_str}'
    ...
    def spawn_gazebo_model(self, robot_params):
        ...
        filename = f'pairs_drone_spawner_{name}.sdf'
...
def main(args=None):
    ...
    spawner_node = PairsDroneSpawner()
    try:
