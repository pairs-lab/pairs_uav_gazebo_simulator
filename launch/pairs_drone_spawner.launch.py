...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
        ...
        default_value = PathJoinSubstitution([
            pkg_pairs_uav_gazebo_simulator, 'config', 'spawner_params.yaml'
        ]),
    ...
    log_config_args = LogInfo(msg=[
        '\n[pairs_drone_spawner.launch.py] Custom config file: ', LaunchConfiguration('custom_config'),
        '\n[pairs_drone_spawner.launch.py] Default config file: ', LaunchConfiguration('default_spawner_config')
    ])
    ld.add_action(log_config_args)

    ld.add_action(
            Node(
                name='pairs_drone_spawner',
                package='pairs_uav_gazebo_simulator',
                executable='pairs_drone_spawner',
                output="screen",
