...
tmux send-keys -t "$P" "$SETUP; "'waitForRos; roslaunch pairs_uav_gazebo_simulation simulation.launch world_name:=pairs_city gui:=true' Enter
P=$(tmux split-window -t "$W_gazebo" -P -F '#{pane_id}')
tmux select-layout -t "$W_gazebo" tiled
tmux send-keys -t "$P" "$SETUP; "'waitForGazebo; waitForSpawn; rosservice call /pairs_drone_spawner/spawn "1 --$UAV_TYPE --enable-rangefinder --enable-realsense-front"' Enter
P=$(tmux split-window -t "$W_gazebo" -P -F '#{pane_id}')
tmux select-layout -t "$W_gazebo" tiled
...
# ---------------- window: vio_data ----------------
read W_vio_data P <<< "$(tmux new-window -t "$SESSION_NAME" -n "vio_data" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForHw; rosrun rqt_image_view rqt_image_view /$UAV_NAME/front_rgbd/color/image_raw' Enter
P=$(tmux split-window -t "$W_vio_data" -P -F '#{pane_id}')
tmux select-layout -t "$W_vio_data" tiled
tmux send-keys -t "$P" "$SETUP; "'waitForHw; rosrun rqt_image_view rqt_image_view /$UAV_NAME/front_rgbd/aligned_depth_to_color/image_raw' Enter
tmux select-layout -t "$W_vio_data" tiled
