...
tmux send-keys -t "$P" "$SETUP; "'waitForGazebo; waitForSpawn; rosservice call /pairs_drone_spawner/spawn "1 --$UAV_TYPE --enable-rangefinder --enable-vio"' Enter
P=$(tmux split-window -t "$W_gazebo" -P -F '#{pane_id}')
tmux select-layout -t "$W_gazebo" tiled
...
tmux send-keys -t "$P" "$SETUP; "'waitForHw; rostopic echo /$UAV_NAME/vio/imu' Enter
P=$(tmux split-window -t "$W_vio_data" -P -F '#{pane_id}')
tmux select-layout -t "$W_vio_data" tiled
tmux send-keys -t "$P" "$SETUP; "'waitForHw; rosrun rqt_image_view rqt_image_view /$UAV_NAME/vio/camera/image_raw' Enter
tmux select-layout -t "$W_vio_data" tiled
