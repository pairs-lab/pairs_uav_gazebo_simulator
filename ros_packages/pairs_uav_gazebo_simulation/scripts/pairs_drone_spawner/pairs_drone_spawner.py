...
from pairs_msgs.srv import String as StringSrv
from pairs_msgs.srv import StringResponse as StringSrvResponse
from pairs_msgs.srv import StringRequest as StringSrvRequest
from pairs_msgs.msg import GazeboSpawnerDiagnostics
...
def exit_handler():
    ...
    '''
    print('[INFO] [PairsDroneSpawner]: Exit requested')

    if len(glob_running_processes) > 0:
        print(f'[INFO] [PairsDroneSpawner]: Shutting down {len(glob_running_processes)} subprocesses')

        num_zombies = 0
        for p in glob_running_processes:
            try:
                p.shutdown()
                print(f'[INFO] [PairsDroneSpawner]: Process {p.run_id} shutdown')
            except:
                num_zombies += 1

        if num_zombies > 0:
            print(f'\033[91m[ERROR] [PairsDroneSpawner]: Could not stop {num_zombies} subprocesses\033[91m')
            exit(1)

    print('[INFO] [PairsDroneSpawner]: Exited gracefully')
    exit(0)
# #}

class PairsDroneSpawner:
    ...
    def __init__(self, verbose=False):
        ...
        rospy.init_node('pairs_drone_spawner', anonymous=True)

        resource_paths = []
        self.rospack = rospkg.RosPack()
        resource_paths.append(os.path.join(self.rospack.get_path('pairs_uav_gazebo_simulation'), 'models'))
        ...
        try:
            ...
            rospy.logerr(f'[PairsDroneSpawner]: Could not load required param {err}')
        ...
        for elem in extra_resource_paths:
            ...
            rospy.loginfo(f'[PairsDroneSpawner]: Adding extra resources from {rpath}')
        ...
        gazebo_simulation_path = self.rospack.get_path('pairs_uav_gazebo_simulation')
        px4_api_path = self.rospack.get_path('pairs_uav_px4_api')
        ...
        try:
            ...
            rospy.logerr(f'[PairsDroneSpawner]: {err}')
            raise rospy.ROSInterruptException

        rospy.loginfo(f'[PairsDroneSpawner]: ------------------------')
        rospy.loginfo(f'[PairsDroneSpawner]: Jinja templates loaded:')
        for name in self.jinja_templates.keys():
            rospy.loginfo(f'[PairsDroneSpawner]: "{name}" from "{self.jinja_templates[name].jinja_template.filename}"')
        rospy.loginfo(f'[PairsDroneSpawner]: ------------------------')
    ...
    def get_all_templates(self):
        ...
        for i, full_name in enumerate(template_names):
            rospy.loginfo(f'[PairsDroneSpawner]: \t({i+1}/{len(template_names)}): {full_name}')
                ...
                for elem in node.body:
                    ...
                    if isinstance(elem, jinja2.nodes.Assign) and elem.target.name == 'spawner_default_args':
                        if isinstance(elem.node, jinja2.nodes.Const):
                            ...
                            rospy.logwarn(f'[PairsDroneSpawner]: Unsupported param type "{type(elem.node)}" in template {template.filename}')
    ...
    def build_template_database(self):
        ...
        rospy.loginfo('[PairsDroneSpawner]: Loading all templates')
        ...
        rospy.loginfo('[PairsDroneSpawner]: Reindexing imported templates')
        ...
        rospy.loginfo('[PairsDroneSpawner]: Adding available components from dependencies')
        ...
        rospy.loginfo('[PairsDroneSpawner]: Pruning components to only include callables')
        callable_components = {}
        for name, template in all_templates:
        ...
        rospy.loginfo('[PairsDroneSpawner]: Template database built')
    ...
    def render(self, spawner_args):
        ...
        try:
            ...
            rospy.logerr(f'[PairsDroneSpawner]: Cannot render template, model not specified')
            return

        try:
            template_wrapper = self.jinja_templates[model_name]
        except KeyError:
            rospy.logerr(f'[PairsDroneSpawner]: Cannot render model "{model_name}". Template not found!')
            return

        rospy.loginfo(f'[PairsDroneSpawner]: Rendering model "{model_name}" using template {template_wrapper.jinja_template.filename}')
        ...
        try:
            ...
            rospy.logerr(f'[PairsDroneSpawner]: XML error: "{e}"')
            fd, filepath = tempfile.mkstemp(prefix='pairs_drone_spawner_' + datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S_"), suffix='_DUMP_' + str(model_name) + '.sdf')
            with os.fdopen(fd, 'w') as output_file:
                output_file.write(rendered_template)
                rospy.loginfo(f'[PairsDroneSpawner]: Malformed XML for model {model_name} dumped to {filepath}')
            return
    ...
    def parse_user_input(self, input_str):
        ...
        for ID in input_dict['ids']:
            if not isinstance(ID, int):
                if ID in self.jinja_templates.keys() and input_dict['model'] is None:
                    rospy.loginfo(f'[PairsDroneSpawner]: Using {ID} as model template')
                    input_dict['model'] = ID
                else:
                    rospy.logwarn(f'[PairsDroneSpawner]: Ignored ID {ID}: Not an integer')
                continue
            if ID < 0 or ID > 255:
                rospy.logwarn(f'[PairsDroneSpawner]: Ignored ID {ID}: Must be in range(0, 256)')
                continue
            if ID in self.assigned_ids:
                rospy.logwarn(f'[PairsDroneSpawner]: Ignored ID {ID}: Already assigned')
                continue
            valid_ids.append(ID)
        ...
        if len(valid_ids) > 0:
            rospy.loginfo(f'[PairsDroneSpawner]: Valid robot IDs: {valid_ids}')
        ...
        if 'pos' in input_dict.keys():
            try:
                ...
                rospy.logerr(f'[PairsDroneSpawner]: While parsing args for "--pos": {err}')
                rospy.logwarn(f'[PairsDroneSpawner]: Assigning random spawn poses instead')
            ...
            try:
                ...
                rospy.logerr(f'[PairsDroneSpawner]: While parsing args for "--pos-file": {err}')
                rospy.logwarn(f'[PairsDroneSpawner]: Assigning random spawn poses instead')
                input_dict['spawn_poses'] = self.get_randomized_spawn_poses(input_dict['ids'])
            finally:
    ...
    def get_model_help_text(self, model_name):
        ...
        Component name
            ...
            Default args:
        '''
        rospy.loginfo(f'[PairsDroneSpawner]: Getting help for model {model_name}')
        try:
            template_wrapper = self.jinja_templates[model_name]
            response = f'[PairsDroneSpawner]: Components used in template "{template_wrapper.jinja_template.filename}":\n'
        except ValueError:
    ...
    def get_spawner_help_text(self):
        ...
        rospy.loginfo(f'[PairsDroneSpawner]: Getting generic spawner help')
    ...
    def callback_spawn(self, req):
        ...
        rospy.loginfo(f'[PairsDroneSpawner]: Spawn called with args "{req.value}"')
        ...
        try:
            ...
            rospy.logwarn(f'[PairsDroneSpawner]: While parsing user input: {e}')
        ...
        rospy.loginfo(f'[PairsDroneSpawner]: Spawner params assigned "{params_dict}"')

        # #{ check gazebo running
        try:
            rospy.loginfo('[PairsDroneSpawner]: Waiting for /gazebo/model_states')
        ...
        rospy.loginfo('[PairsDroneSpawner]: Adding vehicles to a spawn queue')
    ...
    def callback_action_timer(self, timer):
        ...
        if len(self.vehicle_queue) > 0:
            ...
            rospy.loginfo(f'[PairsDroneSpawner]: Vehicle {robot_params["name"]} successfully spawned')
    ...
    def assign_free_id(self):
        ...
        for i in range(0, 256): # 255 is a hard limit of px4 sitl
            if i not in self.assigned_ids:
                rospy.loginfo(f'[PairsDroneSpawner]: Assigned free ID "{i}" to a robot')
                return i
    ...
    def get_spawn_poses_from_file(self, filename, ids):
        ...
        rospy.loginfo(f'[PairsDroneSpawner]: Loading spawn poses from file "{filename}"')
        ...
        rospy.loginfo(f'[PairsDroneSpawner]: Spawn poses returned: {spawn_poses}')
        return spawn_poses
    # #}
    ...
    def get_spawn_poses_from_args(self, pos_args, ids):
        ...
        if len(ids) > 1:
            rospy.logwarn(f'[PairsDroneSpawner]: Spawning more than one vehicle with "--pos". Each additional vehicle will be offset by {self.model_spacing} meters in X')
    ...
    def get_jinja_params_for_one_robot(self, params_dict, index, ID):
        ...
        if 'enable_mavlink_gcs' in params_dict.keys():
            ...
            rospy.loginfo(f'[PairsDroneSpawner]: Publishing extra mavlink messages on UDP port {robot_params["mavlink_gcs_udp_port_remote"]}')
    ...
    def launch_px4_firmware(self, robot_params):
        name = robot_params['name']
        rospy.loginfo(f'[PairsDroneSpawner]: Launching PX4 firmware for {name}')
        ...
        if not os.path.exists(romfs_path) or not os.path.isdir(romfs_path):
            rospy.logerr(f'[PairsDroneSpawner]: Could not start PX4 firmware for {name}. ROMFS folder not found')
        ...
        try:
            ...
            rospy.logerr(f'[PairsDroneSpawner]: Could not start PX4 firmware for {name}. Node failed to launch')
            raise CouldNotLaunch('PX4 failed to launch')

        rospy.loginfo(f'[PairsDroneSpawner]: PX4 firmware for {name} launched')
    ...
    def launch_mavros(self, robot_params):
        name = robot_params['name']
        rospy.loginfo(f'[PairsDroneSpawner]: Launching mavros for {name}')
        ...
        try:
            ...
            rospy.logerr(f'[PairsDroneSpawner]: Could not start mavros for {name}. Node failed to launch')
            raise CouldNotLaunch('Mavros failed to launch')

        rospy.loginfo(f'[PairsDroneSpawner]: Mavros for {name} launched')
    ...
    def spawn_gazebo_model(self, robot_params):
        ...
        if self.save_sdf_files:
            fd, filepath = tempfile.mkstemp(prefix='pairs_drone_spawner_' + datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S_"), suffix='_' + str(robot_params['model']) + '_' + str(name) + '.sdf')
            with os.fdopen(fd, 'w') as output_file:
                output_file.write(sdf_content)
                rospy.loginfo(f'[PairsDroneSpawner]: Model for {name} written to {filepath}')
        ...
        rospy.loginfo(f'[PairsDroneSpawner]: Spawning gazebo model for {name}')
        try:
            response = self.gazebo_spawn_proxy(name, sdf_content, name, spawn_pose, "")
            if not response.success:
                rospy.logerr(f'PairsDroneSpawner]: While calling {self.gazebo_spawn_proxy.resolved_name}: {response.status_message}')
                raise CouldNotLaunch(response.status_message)
        except rospy.service.ServiceException as e:
                rospy.logerr(f'[PairsDroneSpawner]: While calling {self.gazebo_spawn_proxy.resolved_name}: {e}')
    ...
    def delete_gazebo_model(self, name):
        rospy.loginfo(f'[PairsDroneSpawner]: Deleting gazebo model {name}')
        try:
            response = self.gazebo_delete_proxy(name)
            if not response.success:
                rospy.logerr(f'PairsDroneSpawner]: While calling {self.gazebo_delete_proxy.resolved_name}: {response.status_message}')
                return
        except rospy.service.ServiceException as e:
                rospy.logerr(f'[PairsDroneSpawner]: While calling {self.gazebo_delete_proxy.resolved_name}: {e}')
                return

        rospy.loginfo(f'[PairsDroneSpawner]: Model {name} deleted')
    # #}

if __name__ == '__main__':

    print('[INFO] [PairsDroneSpawner]: Starting')
    ...
    try:
        spawner = PairsDroneSpawner(verbose)
